...
Pega `firebase-rules-v1.3.5.json` y publica.
...
- Requiere publicar las reglas `firebase-rules-v1.3.5.json` porque se añade el campo opcional `geoStatus`.
- No modifica ni borra coches, claves, aparcamientos ni vinculaciones.

## Corrección v1.3.5

La causa del error `download_or_parse_error` era concreta: los GeoJSON de geoBoundaries están almacenados
con Git LFS. El CDN usado por la v1.3.3 podía devolver el pequeño fichero puntero de Git LFS en lugar del
GeoJSON real, por lo que `JSON.parse()` fallaba.

La v1.3.5 elimina esa dependencia:

- España: usa `es-atlas` 0.6.0, TopoJSON generado a partir de datos del IGN.
  El navegador convierte localmente TopoJSON a polígonos y obtiene provincia + municipio.
- Brasil: usa `br-geojson`, generado a partir de datos del IBGE.
  Primero localiza el estado y después descarga únicamente los municipios de ese estado.
- Las coordenadas del GPS siguen sin enviarse a Firebase ni a un servicio de geocodificación.
- Se mantiene el diagnóstico geográfico.
- Las reglas Firebase no cambian respecto a v1.3.3.
- No se modifican coches, claves, aparcamientos ni vinculaciones.

## Corrección v1.3.5

Causa definitiva del fallo geográfico:
la Content Security Policy (CSP) de `index.html` solo permitía conexiones a Firebase y al propio dominio.
Por tanto el navegador bloqueaba las descargas de datos administrativos desde `unpkg.com` y `cdn.jsdelivr.net`
antes incluso de que el código pudiera procesarlas.

Cambios:
- `connect-src` permite ahora `https://unpkg.com` para España.
- `connect-src` permite ahora `https://cdn.jsdelivr.net` para Brasil.
- Se mantiene el procesamiento local de las coordenadas.
- Firebase sigue recibiendo solo país, región y municipio, nunca latitud/longitud.
- Actualizado el texto de créditos geográficos.
- Las reglas Firebase no cambian respecto a v1.3.4/v1.3.3.
- No se modifican coches, claves, aparcamientos ni vinculaciones.
